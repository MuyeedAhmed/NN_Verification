...
X = X[15:20]
y = y_predict[15:20]
# print(y_true[15:20].reshape(1,-1))
...
for i in range(len(X)):
    ...
    model.addConstr(Z3[i, 0] <= -0.00001 + M * y_g[i], f"Z3_{i}_upper_bound")
    model.addConstr(y_g[i] == y[i], f"y2_{i}_match_y")

    

model.setObjective(
    gp.quicksum(Z3[i, 0] for i in range(len(X)) if y[i] == 1) - 
    gp.quicksum(Z3[i, 0] for i in range(len(X)) if y[i] == 0) + 
