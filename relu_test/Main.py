...
if __name__ == "__main__":
    ...
    if Gurobi_output is None:
        ...
        sys.exit(1)
    W_h_new, b_h_new, W_new, b_new = Gurobi_output
    TM_after_g.delete_fc_inputs()

    if W_h_new is not None:
        if isinstance(TM_after_g.model.fc_hidden, nn.Sequential):
            lin_h, bn_h = None, None
            for layer in TM_after_g.model.fc_hidden:
            ...
            if lin_h is not None:
                new_W_h = torch.tensor(W_h_new).to(lin_h.weight.device).to(lin_h.weight.dtype)
                new_b_h = torch.tensor(b_h_new).to(lin_h.bias.device).to(lin_h.bias.dtype)
                with torch.no_grad():
                    lin_h.weight.copy_(new_W_h)
                    lin_h.bias.copy_(new_b_h)
                    if bn_h is not None:
                        bn_h.weight.fill_(1.0)
                        bn_h.bias.fill_(0.0)
                        bn_h.running_mean.fill_(0.0)
                        bn_h.running_var.fill_(1.0 - bn_h.eps)
        else:
            new_W_h = torch.tensor(W_h_new).to(TM_after_g.model.fc_hidden.weight.device).to(TM_after_g.model.fc_hidden.weight.dtype)
            new_b_h = torch.tensor(b_h_new).to(TM_after_g.model.fc_hidden.bias.device).to(TM_after_g.model.fc_hidden.bias.dtype)
            with torch.no_grad():
                TM_after_g.model.fc_hidden.weight.copy_(new_W_h)
                TM_after_g.model.fc_hidden.bias.copy_(new_b_h)

    new_W = torch.tensor(W_new).to(TM_after_g.model.classifier.weight.device).to(TM_after_g.model.classifier.weight.dtype)
    new_b = torch.tensor(b_new).to(TM_after_g.model.classifier.bias.device).to(TM_after_g.model.classifier.bias.dtype)
    with torch.no_grad():
