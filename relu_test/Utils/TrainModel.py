...
class TrainModel:
    ...
    def save_model(self, loss, save_suffix=""):
        ...
        if hasattr(self.model, 'fc_hidden'):
            fc_h = self.model.fc_hidden
            w_h, b_h = None, None
            
            if isinstance(fc_h, nn.Linear):
                w_h = fc_h.weight.data.clone()
                b_h = fc_h.bias.data.clone()
            elif isinstance(fc_h, nn.Sequential):
                lin, bn = None, None
                for layer in fc_h:
                ...
                if lin is not None:
                    w_h = lin.weight.data.clone()
                    b_h = lin.bias.data.clone()
                    if bn is not None:
                        ...
                        w_h = w_h * (gamma / std).unsqueeze(1)
                        b_h = (b_h - mean) * (gamma / std) + beta

            if w_h is not None:
                torch.save(w_h, f"{checkpoint_dir}/Run{self.run_id}_fc_hidden_weight{save_suffix}.pt")
                torch.save(b_h, f"{checkpoint_dir}/Run{self.run_id}_fc_hidden_bias{save_suffix}.pt")
