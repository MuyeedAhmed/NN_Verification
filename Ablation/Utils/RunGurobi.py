...
class MILP:
    ...
    def LoadInputs(self, weights_path=None, bias_path=None):
        if self.X_full is None:
            self.X_full = torch.load(f"checkpoints_inputs/{self.dataset_name}/fc_inputs_train.pt", weights_only=True).numpy()
            self.labels_full = torch.load(f"checkpoints_inputs/{self.dataset_name}/fc_labels_train.pt", weights_only=True).numpy()
            self.pred_full = torch.load(f"checkpoints_inputs/{self.dataset_name}/fc_preds_train.pt", weights_only=True).numpy()
        
        if self.input_type == 'v':
            if self.X_val is None:
                self.X_val = torch.load(f"checkpoints_inputs/{self.dataset_name}/fc_inputs_val.pt", weights_only=True).numpy()
                self.labels_val = torch.load(f"checkpoints_inputs/{self.dataset_name}/fc_labels_val.pt", weights_only=True).numpy()
                self.pred_val = torch.load(f"checkpoints_inputs/{self.dataset_name}/fc_preds_val.pt", weights_only=True).numpy()
            source_X = self.X_val
        ...
        if weights_path is None:
            weights_path = f"checkpoints_{self.training_type}/{self.dataset_name}/Run{self.run_id}_classifier_weight.pt"
        if bias_path is None:
            bias_path = f"checkpoints_{self.training_type}/{self.dataset_name}/Run{self.run_id}_classifier_bias.pt"

        self.W = torch.load(weights_path, map_location=torch.device('cpu'), weights_only=True).numpy()
        self.b = torch.load(bias_path, map_location=torch.device('cpu'), weights_only=True).numpy()
    ...
    def Optimize(self, Method = "MisCls_Correct", optimization_direction = "minimize", weights_path=None, bias_path=None):
        milp_log_file = f"Stats/MILP_log.csv" if self.candidate == 0 else f"Stats_Ensemble/MILP_log.csv"
        self.LoadInputs(weights_path=weights_path, bias_path=bias_path)
        self.PrintShapes()
