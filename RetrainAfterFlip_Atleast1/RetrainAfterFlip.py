...
def RunForward(Test, file_name, nn, X, y, y_gt, tol, n, flipCount, l1, l2):
    ...
    for i in range(n):
        ...
        model.addConstr(f[i] == y_scalar + y_g[i] - 2 * y_scalar * y_g[i], name=f"flip_logic_{i}")

        model.addGenConstrIndicator(y_g[i], True,  Z3[i, 0] >= E, name=f"Z3_to_yg_1_{i}")
        model.addGenConstrIndicator(y_g[i], False, Z3[i, 0] <= -E, name=f"Z3_to_yg_0_{i}")        
...
if __name__ == "__main__":
    ...
    Test = f"RetrainAfterFlipAtleast1_l{l1}{l2}"
