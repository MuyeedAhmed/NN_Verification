...
X_test = X[15:20]
y_test = predictions[15:20]
# print("Y true", y[15:20].reshape(1, -1))
    ...
    if solver.check() == sat:
        ...
        for i in range(len(W1_offset)):
            ...
            for j in range(len(W1_offset[0])):
                limitW1[i][j] = model[W1_offset[i][j]].as_fraction() / nn.W1[i][j]
        ...
        for i in range(len(W2_offset)):
            ...
            for j in range(len(W2_offset[0])):
                limitW2[i][j] = model[W2_offset[i][j]].as_fraction() / nn.W2[i][j]
        ...
        for i in range(len(W3_offset)):
            ...
            for j in range(len(W3_offset[0])):
                limitW3[i][j] = model[W3_offset[i][j]].as_fraction() / nn.W3[i][j]
        ...
        print("b1: [", end='')
        for i in range(l1_size):
            limitb1[i] = model[b1_offset[i]].as_fraction()/nn.b1[0, i]
            print(model[b1_offset[i]].as_fraction()/nn.b1[0, i], end=", ")
            # print(model[b1_offset[i]].as_fraction()/nn.b1[0, i], end=", ")
        print("]")
        print("b2: [", end='')
        for i in range(l2_size):
            limitb2[i] = model[b2_offset[i]].as_fraction()/nn.b2[0, i]
            print(model[b2_offset[i]].as_fraction()/nn.b2[0, i], end=", ")
            # print(model[b2_offset[i]].as_fraction()/nn.b2[0, i], end=", ")
        print("]")
        print("b3: [", end='')
        for i in range(l3_size):
            limitb3[i] = model[b3_offset[i]].as_fraction()/nn.b3[0, i]
        ...
        print("]")
        print("y true:", predictions[15:20].reshape(1,-1)[0]) 
        high = max(
            max(max(abs(value) for value in row) for row in limitW1),
            max(max(abs(value) for value in row) for row in limitW2),
            max(max(abs(value) for value in row) for row in limitW3),
            max(abs(value) for value in limitb1),
            max(abs(value) for value in limitb2),
            max(abs(value) for value in limitb3)
        )
