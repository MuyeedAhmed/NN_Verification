...
def GurobiFlip(dataset_name, store_file_name, run_id, n=-1, tol = 1e-5, misclassification_count=1):
    ...
    for s in range(n_samples):
        ...
        if labels_gt[s] == label_max:
            ...
            for k in range(l2_size):
                if k != label_max:
                    gurobi_model.addConstr(Z2[label_max] >= Z2[k] + tol, name=f"violation_0flip_{s}_{k}")
...
def GurobiFlip_Restart(dataset_name, store_file_name, run_id, n=-1, tol = 1e-5, misclassification_count=1, restart_no=0):
    X_full = torch.load(f"checkpoints_inputs/{dataset_name}/fc_inputs_train_{restart_no}.pt").numpy()
    labels_full = torch.load(f"checkpoints_inputs/{dataset_name}/fc_labels_train_{restart_no}.pt").numpy()
    pred_full = torch.load(f"checkpoints_inputs/{dataset_name}/fc_preds_train_{restart_no}.pt").numpy()
    X_full_size = X_full.shape[0]
    if n == -1:
        X = X_full
        labels_gt = labels_full
    ...
    W1 = torch.load(f"checkpoints/{dataset_name}/Run{run_id}_fc_hidden_weight_{restart_no}.pt", map_location=torch.device('cpu')).numpy()
    b1 = torch.load(f"checkpoints/{dataset_name}/Run{run_id}_fc_hidden_bias_{restart_no}.pt", map_location=torch.device('cpu')).numpy()
    W2 = torch.load(f"checkpoints/{dataset_name}/Run{run_id}_classifier_weight_{restart_no}.pt", map_location=torch.device('cpu')).numpy()
    b2 = torch.load(f"checkpoints/{dataset_name}/Run{run_id}_classifier_bias_{restart_no}.pt", map_location=torch.device('cpu')).numpy()
    ...
    if gurobi_model.status in [GRB.TIME_LIMIT, GRB.OPTIMAL] and gurobi_model.SolCount > 0:
        ...
        X_val = torch.load(f"checkpoints_inputs/{dataset_name}/fc_inputs_val_{restart_no}.pt").numpy()
        labels_val = torch.load(f"checkpoints_inputs/{dataset_name}/fc_labels_val_{restart_no}.pt").numpy()
        pred_val = torch.load(f"checkpoints_inputs/{dataset_name}/fc_preds_val_{restart_no}.pt").numpy()
        Z1_val = np.maximum(0, X_val @ W1.T + b1)
        Z2_val_pred = Z1_val @ W2_new.T + b2_new
        predictions_val = np.argmax(Z2_val_pred, axis=1)
        accuracy_val = np.sum(predictions_val == labels_val) / len(labels_val) * 100
        ...
        with open(store_file_name, "a") as f:
            f.write(f"{restart_no},GurobiComplete_Train,-1,-1,{accuracy_gurobi}\n")
            f.write(f"{restart_no},GurobiComplete_Val,-1,-1,{accuracy_val}\n")
