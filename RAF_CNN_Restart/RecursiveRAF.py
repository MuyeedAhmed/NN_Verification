...
import random
import numpy as np
from TrainModel_Restart import TrainModel
from medmnist import PathMNIST
from RunGurobi import GurobiFlip_Restart
...
if __name__ == "__main__":
    ...
    for i in range(1, total_run + 1):
        ...
        if os.path.exists(f"./checkpoints/{dataset_name}/Run{i}_full_checkpoint_0.pth") == False:
            TM = TrainModel(method, dataset_name, model_t, train_loader, val_loader, device, num_epochs=initEpoch, batch_size=64, learning_rate=learningRate, optimizer_type=optimize, phase="Train", run_id=i, restart_no=0, start_experiment=start_experiment)
            TM.run()
        else:
            print(f"Run {i} already exists. Skipping training...")
        
        for restart in range(1, total_restart + 1):
            if os.path.exists(f"./checkpoints/{dataset_name}/Run{i}_full_checkpoint_{restart}.pth"):
                print(f"Restart {restart} for run {i} already exists. Skipping...")
                continue

            TM_after_g = TrainModel(method, dataset_name, model_g, train_loader, val_loader, device, num_epochs=G_epoch, batch_size=64, learning_rate=learningRate, optimizer_type=optimize, phase="GurobiEdit", run_id=i, restart_no=restart)
            if device.type == 'cuda':
                checkpoint = torch.load(f"./checkpoints/{dataset_name}/Run{i}_full_checkpoint_{restart-1}.pth")
            else:
                checkpoint = torch.load(f"./checkpoints/{dataset_name}/Run{i}_full_checkpoint_{restart-1}.pth", map_location=torch.device('cpu'))
            ...
            TM_after_g.save_fc_inputs("Train", restart=restart-1)
            TM_after_g.save_fc_inputs("Val", restart=restart-1)
            print(f"Saved FC inputs for run {i}.")
            
            Gurobi_output = GurobiFlip_Restart(dataset_name, TM_after_g.log_file, i, n=n_samples_gurobi, misclassification_count=misclassification_count, restart_no=restart-1)
            ...
            with open(TM_after_g.log_file, "a") as f:
                f.write(f"{restart-1},Gurobi_Complete_Eval_Train,-1,{train_loss},{train_acc}\n")
                f.write(f"{restart-1},Gurobi_Complete_Eval_Val,-1,{val_loss},{val_acc}\n")
            TM_after_g.run()
