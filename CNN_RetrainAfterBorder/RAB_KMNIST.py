...
def TrainAndSave(resume=False):
    ...
    checkpoint_dir = "./checkpoints/KMNIST"
...
def GurobiBorder():
    X = torch.load("checkpoints/KMNIST/fc_inputs.pt").numpy()
    labels = torch.load("checkpoints/KMNIST/fc_labels.pt").numpy()
    pred = torch.load("checkpoints/KMNIST/fc_preds.pt").numpy()

    W1 = torch.load("checkpoints/KMNIST/fc_hidden_weight.pt").cpu().numpy()
    b1 = torch.load("checkpoints/KMNIST/fc_hidden_bias.pt").cpu().numpy()
    W2 = torch.load("checkpoints/KMNIST/classifier_weight.pt").cpu().numpy()
    b2 = torch.load("checkpoints/KMNIST/classifier_bias.pt").cpu().numpy()
    ...
    if model.status in [GRB.TIME_LIMIT, GRB.OPTIMAL] and model.SolCount > 0:
        ...
        checkpoint = torch.load("./checkpoints/KMNIST/full_checkpoint.pth")
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
