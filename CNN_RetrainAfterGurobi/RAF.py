...
def GurobiBorder(dataset_name, n=-1, tol = 5e-6, misclassification_count=1):
    ...
    W2_offset = model_g.addVars(*W2.shape, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="W2_offset")
    b2_offset = model_g.addVars(l2_size, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="b2_offset")

    Z2_list = []
    max_min_diff = []
    misclassified_flags = model_g.addVars(n_samples, vtype=GRB.BINARY, name="misclassified_flags")
    ...
    if model_g.status in [GRB.TIME_LIMIT, GRB.OPTIMAL] and model_g.SolCount > 0:
        ...
        if n != -1:
            ...
            with open(f"Stats_RAF/{dataset_name}_gurobi_log.csv", "a") as f:
...
if __name__ == "__main__":
    ...
    raf_g = RAF(dataset_name, model_g, train_loader, test_loader, device, num_epochs=G_epoch, resume_epochs=0, batch_size=64, learning_rate=0.01, optimizer_type=optimize, phase="GurobiEdit")
    ...
    raf_g.model.load_state_dict(checkpoint['model_state_dict'])
    raf_g.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    raf_g.scheduler.load_state_dict(checkpoint['scheduler_state_dict'])

    new_W = torch.tensor(W2_new).to(model_g.classifier.weight.device)
    new_b = torch.tensor(b2_new).to(model_g.classifier.bias.device)
    with torch.no_grad():
        raf_g.model.classifier.weight.copy_(new_W)
        raf_g.model.classifier.bias.copy_(new_b)
    raf_g.run()