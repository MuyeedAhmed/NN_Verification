...
def train_model(X_train, X_val, y_train, y_val, l1, l2, save_path=None, preset_weights_path=None, max_epochs=500):
...
def RunForward(Test, file_name, nn, X, y, y_gt, tol, n, flipCount, l1, l2):
    ...
    if model.status == GRB.TIME_LIMIT or model.status == GRB.OPTIMAL:
        ...
        f_values = [int(f[i].X) for i in range(len(X))]
        ...
        print("Weight Sum:", sum(np.abs(W1_offset[i, j].X) for i in range(len(nn.W1)) for j in range(l1_size)) +
              sum(np.abs(W2_offset[i, j].X) for i in range(len(nn.W2)) for j in range(l2_size)) +
              sum(np.abs(W3_offset[i, j].X) for i in range(len(nn.W3)) for j in range(l3_size)) +
