...
def RunForward(Test, file_name, nn, X, y, y_gt, tol, n, flipCount, l1, l2):
    ...
    NewW1 = [[nn.W1[i][j] for j in range(l1_size)] for i in range(len(nn.W1))]
    NewW2 = [[nn.W2[i][j] for j in range(l2_size)] for i in range(len(nn.W2))]
    ...
    if model.status == GRB.TIME_LIMIT or model.status == GRB.OPTIMAL:
        ...
        W1_values_with_offset = np.array([[nn.W1[i][j] for j in range(l1_size)] for i in range(len(nn.W1))])
        W2_values_with_offset = np.array([[nn.W2[i][j] for j in range(l2_size)] for i in range(len(nn.W2))])
        ...
        b1_values_with_offset = np.array([nn.b1[0, j] for j in range(l1_size)])
        b2_values_with_offset = np.array([nn.b2[0, j] for j in range(l2_size)])
    ...
    for file_name in os.listdir(dataset_dir):
        ...
        try:
            ...
            try:
                ...
                solution_found = RunForward(Test, file_name, extracted_weights, X_train, y_train_pred, y_train, tol, len(X), flipCount, l1, l2)
            except Exception as e:
                with open(error_file, "a") as f:
                    f.write(f"------------\n{file_name}\nStep 1: {e}\n---------------\n")
                continue
