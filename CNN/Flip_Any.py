...
for s in range(n_samples):
    ...
    # print(np.max(Z3_target[s]))
    # big_M = np.max(Z3_target) - np.min(Z3_target) + 10
    big_M = 1e4
    epsilon = 1e-4
    for k in range(10):
        if k != label:
            model.addConstr(Z3[label] >= Z3[k] + epsilon - big_M * (1 - misclassified_flags[s]),
                            name=f"misclass_logic_{s}_{k}")
            model.addConstr(Z3[label] + big_M * misclassified_flags[s] >= Z3[k] + epsilon,
                            name=f"correct_logic_{s}_{k}")

model.addConstr(gp.quicksum(misclassified_flags[s] for s in range(n_samples)) == 1, name="exactly_one_misclass")
